...
async function validatePluginsOptions(plugins) {
  let errors = 0;
    ...
    try {
      ...
      if ((_plugin$options = plugin.options) === null || _plugin$options === void 0 ? void 0 : _plugin$options.plugins) {
        const {
          errors: subErrors,
          plugins: subPlugins
        } = await validatePluginsOptions(plugin.options.plugins);
...
async function validateConfigPluginsOptions(config = {}) {
  if (!config.plugins) return;
  const {
    errors,
    plugins
  } = await validatePluginsOptions(config.plugins);
